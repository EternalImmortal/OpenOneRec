...
def split_dataframe(df: pd.DataFrame, max_rows: int, output_dir: str, prefix: str = "part", test_chunk_num: int=-1) -> List[str]:
    ...
    for chunk_idx in tqdm(range(num_chunks), desc="Splitting files"):
        if test_chunk_num > 0 and chunk_idx >= test_chunk_num:
            logger.info(f"Test chunk number limit reached: {test_chunk_num}, stopping further splits.")
            break
...
def main():
    ...
    try:
        ...
        output_files = split_dataframe(
            ...
            prefix="part",
            test_chunk_num=args.test_chunk_num,
        )
